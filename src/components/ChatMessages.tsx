// ChatMessages.tsx - MODIFIED FOR RESPONSIVENESS + material-session loading text
...
interface ChatMessagesProps {
  ...
  currentTheme?: string;
  // NEW: true only when chat was started from material analysis
  isMaterialSession?: boolean;
}
...
export const ChatMessages: React.FC<ChatMessagesProps> = (props) => {
  ...
  // Always scroll to bottom after new content renders
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      const container = messagesContainerRef.current;
      if (container) {
        container.scrollTop = container.scrollHeight - container.clientHeight;
        // Or: messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
      }
    }, 100);
  ...
  // - If started from material analysis AND initial explanation not done → "Analyzing your material..."
  // - Else → "Thinking..."
  const loadingText =
    isMaterialSession && !hasReceivedInitialExplanation
  ...
  return (
    <div
      ...
      className="w-full max-w-6xl mx-auto flex-1 overflow-y-auto pb-32 px-4 md:px-6 space-y-4 h-full"
    >
      {messages.map((msg, idx) => (
        <motion.div
          ...
          className={`flex w-full ${
            msg.isBot ? "justify-center" : "justify-end"
          }`}
        >
          <ErrorBoundary FallbackComponent={ErrorFallback}>
            <div
              className={
                msg.isBot
                  ? "max-w-full w-full text-gray-200 bg-black/70 p-4 rounded-xl"
                  : "max-w-md min-w-0 w-fit text-white bg-blue-700 px-3 py-2 rounded-xl"
              }
            >
              <ReactMarkdown
                className="text-base leading-relaxed whitespace-pre-wrap text-left"
              ...
              {msg.videoList && msg.videoList.length > 0 && (
                <div className="mt-4 pt-4 border-t border-gray-500/50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {msg.videoList.map((video: RelatedVideo, videoIdx: number) => {
              ...
              {/* Example PhET embed hook */}
              {renderPhET &&
                msg.text?.includes("PhET") &&
                renderPhET(
